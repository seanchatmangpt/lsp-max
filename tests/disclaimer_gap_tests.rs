        ...
        for e in evidence {
            match e {
                Evidence::MechanicalWitness { receipt } => {
                    ...
                    if receipt.action_digest == valid_action_digest && receipt.observation_digest == valid_obs_digest {
                        valid_receipt = true;
                    } else {
                        return Err(anyhow!("Chatman Equation Failed: Invalid digest in receipt"));
                    }
                }
...
fn model_output_without_receipt_is_candidate() -> Result<()> {
    let observation = BoundedObservation { state: "valid_state".into() };
    let action = Action { mutation: "valid_mutation".into() };
    
    let result = DisclaimerGapGate::evaluate_admission(&action, &observation, &[]);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("DISCLAIMER_GAP_CLOSED"));
    Ok(())
}

#[test]
fn agent_summary_cannot_admit_lsif() -> Result<()> {
    let observation = BoundedObservation { state: "lsif_state".into() };
    let action = Action { mutation: "lsif_mutation".into() };
    
    let evidence = vec![Evidence::AgentSummary { text: "LSIF is good to go".into() }];
...
fn test_output_is_not_receipt() -> Result<()> {
    let observation = BoundedObservation { state: "code_state".into() };
    let action = Action { mutation: "code_mutation".into() };
    
    let evidence = vec![Evidence::TestOutput { stdout: "test passed".into() }];
...
fn candidate_as_authority_refused() -> Result<()> {
    let observation = BoundedObservation { state: "state".into() };
    let action = Action { mutation: "mutation".into() };
    
    let evidence = vec![Evidence::AgentClaim { text: "model output is authority".into() }];
...
fn disclaimer_gap_open_when_no_mechanical_witness() -> Result<()> {
    ...
    let action = Action { mutation: "m".into() };
    
    let result = DisclaimerGapGate::evaluate_admission(&action, &observation, &[]);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("mechanical witness"));
    Ok(())
}
...
fn model_disclaimer_not_treated_as_safety() -> Result<()> {
    ...
    let evidence = vec![Evidence::ModelDisclaimer { text: "I might be wrong".into() }];
...
fn agent_claims_admitted_without_receipt_refused() -> Result<()> {
    let observation = BoundedObservation { state: "lsif".into() };
    let action = Action { mutation: "update".into() };
    
    let evidence = vec![Evidence::AgentClaim { text: "LSIF admitted because the model says the tests passed.".into() }];
...
fn test_disclaimer_gap_closed_invariant() -> Result<()> {
    ...
    let evidence = vec![Evidence::MechanicalWitness { receipt: valid_receipt }];
