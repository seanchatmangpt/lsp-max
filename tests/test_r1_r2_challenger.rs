...
fn test_r1_positive_code_action_serialization() {
    ...
    assert!(
        ...
        "Serialized JSON must contain 'tags'"
    );
    assert!(
        serialized.contains("documentation"),
        "Serialized JSON must contain 'documentation'"
    );
    assert!(
        serialized.contains("disabled"),
        "Serialized JSON must contain 'disabled'"
    );
    assert!(
        serialized.contains("data"),
        "Serialized JSON must contain 'data'"
    );

    let deserialized: CodeAction =
        serde_json::from_str(&serialized).expect("Should deserialize CodeAction");
...
fn test_r1_positive_lsif_graph_types_serialization() {
    ...
    let deserialized: Element =
        serde_json::from_str(&serialized).expect("Should deserialize LSIF element");
    match deserialized {
...
fn test_r1_negative_malformed_lsif_vertex_rejected() {
    ...
    assert!(
        res.is_err(),
        "Should reject non-string version in LSIF MetaData"
    );
}
...
async fn test_r2_bypass_fails_on_static_mock() {
    ...
    if let Some(lsp::HoverProviderCapability::Simple(true)) = caps.hover_provider {
        panic!(
