...
fn fitness_snapshot_admitted_threshold() {
    ...
    assert_eq!(v["law_status"], "ADMITTED", "fitness=0.87, violations=0 must yield ADMITTED");
...
fn fitness_snapshot_candidate_threshold() {
    ...
    let content = fs::read_to_string(&path).unwrap();
    let v: Value = serde_json::from_str(&content).unwrap();
    assert_eq!(v["law_status"], "CANDIDATE", "fitness=0.70, violations=1 must yield CANDIDATE");
}
...
fn fitness_snapshot_blocked_threshold() {
    ...
    let content = fs::read_to_string(&path).unwrap();
    let v: Value = serde_json::from_str(&content).unwrap();
    assert_eq!(v["law_status"], "BLOCKED", "fitness=0.40, violations=5 must yield BLOCKED");
}
...
fn fitness_snapshot_has_required_fields() {
    ...
    let content = fs::read_to_string(&path).unwrap();
    let v: Value = serde_json::from_str(&content).unwrap();
    for field in &["fitness", "precision", "declare_violations", "ocel_event_count", "law_status"] {
        assert!(v.get(field).is_some(), "fitness snapshot missing field: {field}");
    }
}
...
fn gate_check_emits_structured_json_on_block() {
    ...
    let hook = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join(".claude/hooks/gate-check.sh");
    if !hook.exists() {
    ...
    let path_env = format!("{}:{}", mock_dir.display(), std::env::var("PATH").unwrap_or_default());
    ...
    assert_eq!(output.status.code(), Some(1), "gate-check.sh must exit 1 when blocked");
...
fn violations_detail_in_fitness_file() {
    ...
    let content = fs::read_to_string(&path).unwrap();
    let v: Value = serde_json::from_str(&content).unwrap();
    let violations = v["violations"].as_array().expect("violations must be an array");
    assert_eq!(violations.len(), 2, "violation count must match array length");
    let first = &violations[0];
    assert!(first.get("constraint").is_some(), "violation missing 'constraint' field");
    assert!(first.get("case_id").is_some(), "violation missing 'case_id' field");
    assert!(first.get("detail").is_some(), "violation missing 'detail' field");
}
...
fn gate_check_includes_first_violation() {
    ...
    let parsed: Value = serde_json::from_str(stdout.trim())
        .unwrap_or_else(|_| panic!("gate-check.sh stdout is not valid JSON: {stdout:?}"));
    assert_eq!(parsed["decision"], "block");
    let fv = parsed.get("first_violation").expect("first_violation must be present when violations exist");
    assert!(fv.get("constraint").is_some(), "first_violation missing constraint");
