# Quality & Gate Policies: tower-lsp-max v26.6.5
...
3. **E2E**: Spins up the full `tower-lsp-max` LSP server process, indexes a sample workspace folder, and triggers JSON-RPC queries to verify that definition/reference lookup responses include correct cryptographic receipts.
...
| **RSK-003** | **LSIF-RDF Ontology Drift**: Structural changes to the physical LSIF schema mismatch the RDF predicate definitions. | High | Generating Rust structure models and RDF schema mappings from a single source definition using the `tower-lsp-max-specgen` tool. |
