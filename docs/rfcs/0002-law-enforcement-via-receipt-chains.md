...
A law violation detected *after* a system has been deployed is far more costly than one caught *during* development. Thus, auditability at the point of decision is non-negotiable.

## Decision

Enforce law compliance via **receipt chains**: immutable ledger entries that form a cryptographic hash chain (BLAKE3 boundaries and digests). Each receipt captures:

1. **Boundary:** The point in the execution timeline where a decision was made.
2. **Digest:** A BLAKE3 hash that commits to the decision, its inputs, and the law-axis that governs it.
...
Tools and gates may refuse execution unless a receipt chain supports the claim. Verification tools can check that code attempting to claim "admission" actually produced a receipt artifact with a valid chain.

## Rationale

Receipt chains provide:

1. **Auditability:** Every law-bearing decision leaves an immutable trace.
2. **Forensic recovery:** Violations can be traced backward through the chain to the root cause.
3. **Testability:** Unit tests and CI can verify that receipt chains are present and valid before accepting claims.
4. **Replay safety:** A receipt cannot be forged or reused without recomputing the entire chain.
5. **Determinism:** Cryptographic validation is deterministic and side-channel resilient; there is no hidden state or timing-dependent behavior.
...
- Verification overhead: computing BLAKE3 chains on every state transition incurs CPU cost.
...
- **Type definitions:** `lsp-max-protocol/src/receipt.rs`, `lsp-max-runtime/src/control_plane/receipts.rs`
- **Diagnostic family:** `LAW-*`
- **Key crate:** `lsp-max-protocol` (receipt types), `lsp-max-runtime` (receipt generation and verification)
- **Verification:** `src/gate.rs` (gate check logic validates receipt chains)
