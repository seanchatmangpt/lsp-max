...
pub fn extract_workspaces_from_store(
    ...
    for quad in store.quads_for_pattern(None, Some(rdf_type.as_ref()), Some(doc_type.as_ref()), Some(graph_name.as_ref())) {
        let quad = quad.map_err(|e| e.to_string())?;
        for uri_quad in store.quads_for_pattern(Some(quad.subject.as_ref()), Some(uri_pred.as_ref()), None, Some(graph_name.as_ref())) {
    ...
    for quad in store.quads_for_pattern(None, Some(rdf_type.as_ref()), Some(project_type.as_ref()), Some(graph_name.as_ref())) {
        let quad = quad.map_err(|e| e.to_string())?;
        workspaces.push(WorkspaceEvidencePayload { id: quad.subject.to_string(), kind: "Project".to_string(), document_uris: document_uris.clone() });
    }

    if workspaces.is_empty() && !document_uris.is_empty() {
        workspaces.push(WorkspaceEvidencePayload { id: "urn:tower-lsp-max:workspace:default".to_string(), kind: "Workspace".to_string(), document_uris });
    }
    Ok(workspaces)
}
...
pub fn extract_ranges_from_store(
    ...
    let rdf_type = oxigraph::model::NamedNode::new("http://www.w3.org/1999/02/22-rdf-syntax-ns#type").unwrap();
    ...
    fn lit_u32(store: &oxigraph::store::Store, subject: oxigraph::model::SubjectRef, pred: &oxigraph::model::NamedNode, graph: &oxigraph::model::GraphName) -> u32 {
        store.quads_for_pattern(Some(subject), Some(pred.as_ref()), None, Some(graph.as_ref()))
            .flatten()
            .find_map(|q| if let oxigraph::model::Term::Literal(l) = q.object { l.value().parse().ok() } else { None })
            .unwrap_or(0)
    }
    ...
    for quad in store.quads_for_pattern(None, Some(rdf_type.as_ref()), Some(range_type.as_ref()), Some(graph_name.as_ref())) {
        let quad = quad.map_err(|e| e.to_string())?;
...
pub fn extract_diagnostics_from_store(
    ...
    let rdf_type = oxigraph::model::NamedNode::new("http://www.w3.org/1999/02/22-rdf-syntax-ns#type").unwrap();
    let diag_type = oxigraph::model::Term::NamedNode(oxigraph::model::NamedNode::new("urn:tower-lsp-max:core:Diagnostic").unwrap());
    ...
    fn lit_str(store: &oxigraph::store::Store, subject: oxigraph::model::SubjectRef, pred: &oxigraph::model::NamedNode, graph: &oxigraph::model::GraphName) -> Option<String> {
        store.quads_for_pattern(Some(subject), Some(pred.as_ref()), None, Some(graph.as_ref()))
            .flatten()
            .find_map(|q| if let oxigraph::model::Term::Literal(l) = q.object { Some(l.value().to_string()) } else { None })
    }

    let mut diagnostics = Vec::new();
    for quad in store.quads_for_pattern(None, Some(rdf_type.as_ref()), Some(diag_type.as_ref()), Some(graph_name.as_ref())) {
        let quad = quad.map_err(|e| e.to_string())?;
        let subj = quad.subject.as_ref();
...
pub fn extract_receipts_from_store(
    ...
    let rdf_type = oxigraph::model::NamedNode::new("http://www.w3.org/1999/02/22-rdf-syntax-ns#type").unwrap();
    let rcpt_type = oxigraph::model::Term::NamedNode(oxigraph::model::NamedNode::new("urn:tower-lsp-max:core:Receipt").unwrap());
    let prev_hash_pred = oxigraph::model::NamedNode::new("urn:tower-lsp-max:core:prevHash").unwrap();
    let result_hash_pred = oxigraph::model::NamedNode::new("urn:tower-lsp-max:core:resultHash").unwrap();
    let query_hash_pred = oxigraph::model::NamedNode::new("urn:tower-lsp-max:core:queryHash").unwrap();
    let graph_hash_pred = oxigraph::model::NamedNode::new("urn:tower-lsp-max:core:graphHash").unwrap();

    fn lit_str(store: &oxigraph::store::Store, subject: oxigraph::model::SubjectRef, pred: &oxigraph::model::NamedNode, graph: &oxigraph::model::GraphName) -> String {
        store.quads_for_pattern(Some(subject), Some(pred.as_ref()), None, Some(graph.as_ref()))
            .flatten()
            .find_map(|q| if let oxigraph::model::Term::Literal(l) = q.object { Some(l.value().to_string()) } else { None })
            .unwrap_or_default()
    ...
    for quad in store.quads_for_pattern(None, Some(rdf_type.as_ref()), Some(rcpt_type.as_ref()), Some(graph_name.as_ref())) {
        let quad = quad.map_err(|e| e.to_string())?;
        let subj = quad.subject.as_ref();
