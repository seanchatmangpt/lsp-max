...
pub struct GateStatus {
    ...
    /// False when the compositor has not written the gate file yet.
...
/// Check the compositor ANDON gate. Exit 1 if ANDON is set; 0 if clear.
/// Reads one byte — no IPC, no subprocess — safe in PreToolUse hooks.
#[verb("check")]
pub fn check() -> Result<GateStatus> {
    let status = GateService::new().check();
...
mod tests {
    ...
    #[test]
    fn gate_check_clear_when_no_compositor() {
