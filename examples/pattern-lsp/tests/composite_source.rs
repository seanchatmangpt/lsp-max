...
fn test_composite_source_merging() {
    ...
    let mut pattern_diagnostics: Vec<Diagnostic> = pattern_findings
        .into_iter()
        .map(|f| Diagnostic {
    ...
    let has_ra = composite
        .iter()
        .any(|d| d.source.as_deref() == Some("rust-analyzer"));
    let has_pattern = composite
        .iter()
        .any(|d| d.source.as_deref() == Some("pattern-lsp"));

    assert!(has_ra, "Composite should contain rust-analyzer diagnostics");
    assert!(
        has_pattern,
        "Composite should contain pattern-lsp diagnostics"
    );
    ...
    assert_ne!(
        composite.len(),
        composite_disabled.len(),
        "Diagnostic count must change when pattern-lsp is disabled"
    );
}
