...
fn walk_fn_body(
    ...
    match kind {
        ...
        "match_expression" => {
            metrics.branch_count += 1;
            let arm_count = node.children(&mut node.walk()).filter(|c| c.kind() == "match_arm").count();
            if arm_count > metrics.max_match_arms {
                metrics.max_match_arms = arm_count;
            }
        }
        "binary_expression" => {
            let op = node.child(1).and_then(|c| c.utf8_text(source).ok()).unwrap_or_default().to_string();
            if op == "&&" || op == "||" {
                metrics.branch_count += 1;
            }
    ...
    // Detect transmute
    if text.contains("mem::transmute") || (kind == "call_expression" && text.starts_with("transmute")) {
    ...
    if kind == "call_expression" && (text.contains("inference_trace.push") || text.contains("trace.push")) {
    ...
    let new_closure_depth = if kind == "closure_expression" { closure_depth + 1 } else { closure_depth };
...
fn collect_fn_metrics(node: Node, source: &[u8], filepath: &str) -> Vec<Observation> {
    ...
    if metrics.branch_count > 10 {
        obs.push(make("fn_high_cyclomatic", format!("Function '{}' has cyclomatic complexity {} (threshold 10)", name, metrics.branch_count + 1)));
    }
    // METRIC-003: deep nesting
    if metrics.max_depth > 4 {
        obs.push(make("fn_deep_nesting", format!("Function '{}' max nesting depth {} (threshold 4)", name, metrics.max_depth)));
    }
    // METRIC-004: literal-dense (blocking — oracle lookup table signal)
    if literal_ratio > 0.35 && metrics.leaf_count > 10 {
        obs.push(make("fn_literal_dense", format!("Function '{}' literal density {:.0}% (threshold 35%)", name, literal_ratio * 100.0)));
    }
    // METRIC-005: large match dispatch (blocking — array dispatch oracle)
    if metrics.max_match_arms > 20 {
        obs.push(make("fn_large_match", format!("Function '{}' has match with {} arms (threshold 20)", name, metrics.max_match_arms)));
    }
    // METRIC-006: deep closures
    if metrics.max_closure_depth > 3 {
        obs.push(make("fn_deep_closures", format!("Function '{}' closure nesting depth {} (threshold 3)", name, metrics.max_closure_depth)));
    }

    // ORACLE-002: transmute
    if metrics.has_transmute {
        obs.push(make("transmute_cast", format!("Function '{}' uses mem::transmute or raw pointer cast", name)));
    }
    // ORACLE-004: env var in prod
    if metrics.has_env_var {
        obs.push(make("env_var_in_prod", format!("Function '{}' reads env var in production path", name)));
    }
    // ORACLE-001: lazy_static env
    if metrics.has_lazy_static_env {
        obs.push(make("lazy_static_env_init", format!("Function '{}' initializes lazy_static from env (oracle injection)", name)));
    }
    // ORACLE-003: global hashmap literal
    if metrics.has_global_hashmap_literal {
        obs.push(make("global_hashmap_literal", format!("Function '{}' builds HashMap from string literal keys (memo oracle)", name)));
    }
    ...
    if !metrics.trace_push_strings.is_empty() {
        obs.push(make("trace_constant_push", format!("Function '{}' pushes {} constant string(s) to inference_trace", name, metrics.trace_push_strings.len())));
    }
    // TRACE-002: hardcoded trace len assert
    if metrics.trace_len_asserts > 0 {
        obs.push(make("trace_len_magic_assert", format!("Function '{}' asserts trace.len() == literal integer", name)));
    }
    ...
    if matches!(name.as_str(), "run" | "compute" | "infer" | "execute" | "evaluate") {
        if halstead_vocab < 10 && metrics.leaf_count > 5 {
            obs.push(make("halstead_low_volume", format!("Function '{}' has Halstead vocabulary {} (< 10) — oracle injection signal", name, halstead_vocab)));
        }
        if n2 < 5 && metrics.leaf_count > 5 {
            obs.push(make("halstead_low_vocabulary", format!("Function '{}' has only {} distinct operands — memorization signal", name, n2)));
        }
        if n1 > 0 && n2 > 0 && (n2 as f32 / n1 as f32) < 0.3 {
            obs.push(make("halstead_operator_dominance", format!("Function '{}' operator:operand ratio {:.2} (< 0.3) — control-only oracle", name, n2 as f32 / n1 as f32)));
        }
    }

    // CONTRACT-003: cfg(test) fn named run/compute
    if metrics.cfg_test_run_fn {
        obs.push(make("cfg_test_run_fn", format!("'{}' inside #[cfg(test)] matches production API name", name)));
    }
...
fn traverse_node(node: Node, source: &[u8], filepath: &str, obs: &mut Vec<Observation>) {
    ...
    if kind == "const_item" || kind == "static_item" {
        if text.contains("f64") || text.contains("f32") || text.contains('.') {
            for word in text.split_whitespace() {
                ...
                if is_oracle_float(clean) {
                    obs.push(Observation {
                        ...
                        context: text.chars().take(120).collect(),
                        message: format!("Static/const item contains float in known oracle value range"),
                    });
                    break;
                }
            }
        ...
        if cursor.goto_first_child() {
            loop {
                ...
                if child.kind() == "function_item" {
                    if let Some(body) = child.children(&mut child.walk()).find(|c| c.kind() == "block") {
                        let stmt_count = body.children(&mut body.walk()).filter(|c| {
                            !matches!(c.kind(), "{" | "}" | "comment")
                        }).count();
                        if stmt_count <= 2 {
                            let fn_name = child.children(&mut child.walk())
                                .find(|c| c.kind() == "identifier")
                                .and_then(|c| c.utf8_text(source).ok())
                                .unwrap_or("(anon)");
                            obs.push(Observation {
