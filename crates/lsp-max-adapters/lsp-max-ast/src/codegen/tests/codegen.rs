...
fn test_codegen_dry_run() {
    let _lock = crate::codegen::TEST_LOCK.lock().unwrap();
    let source = r#"[{"type": "identifier", "named": true}]"#;
    let _result = crate::codegen::generate(source, &tree_sitter_html::LANGUAGE.into(), None);
...
fn gen_python() {
    let _lock = crate::codegen::TEST_LOCK.lock().unwrap();
    let _result = crate::codegen::generate(
