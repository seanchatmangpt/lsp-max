...
pub struct Builder {
    id_ctr: usize,
    /// Vector storing built AST nodes.
    ///
    /// WARNING: Storing nodes as trait objects (`Vec<Box<dyn AstNode>>`) introduces
    /// a potential memory pointer chasing regression risk. Each dynamic dispatch
    /// requires dereferencing the box, which can degrade CPU cache locality and lead to
    /// latency overheads under tight traversal loops compared to contiguous enum vectors.
