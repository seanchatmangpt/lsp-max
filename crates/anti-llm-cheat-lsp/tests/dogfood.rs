...
fn check_diag_code(diags: &[anti_llm_cheat_lsp::diagnostics::AntiLlmDiagnostic], expected: &str) {
    ...
    // Verify against the LSP-MAX ANDON Push Harness (DoD Step 8)
    let mut harness = lsp_max::andon::test::LspMaxHarness::new();
    for d in diags {
        harness.push_event(d.to_andon_event());
    }

    harness.assert_andon(expected);
    harness.assert_diagnostic(expected);

    let target = target.unwrap();
    if target.blocking {
        harness.assert_admission_disabled();
        harness.assert_next_lawful_step(&target.required_correction);
    }
}
