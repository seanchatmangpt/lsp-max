...
/// (nodes, edges); each tuple is (id, kind, label) from `graph`.
type GraphData = (Vec<(String, String, String)>, Vec<(String, String, String)>);

/// (regressed, improved, unchanged_count); each row is
/// (instance_id, baseline_score, current_score, delta) from `diff_baseline`.
type BaselineDiff = (
...
impl WorkspaceService {
                    ...
                    for diag in &inst.diagnostics {
                        for to_id in &instance_ids {
                            if to_id != from_id && diag.lsp.message.contains(to_id.as_str()) {
                                let edge =
                                    (from_id.clone(), to_id.clone(), "DIAGNOSTIC_REF".to_string());
    ...
    pub fn diff_baseline(&self) -> std::result::Result<BaselineDiff, String> {
        let path = Self::baseline_path();
        let content =
            fs::read_to_string(&path).map_err(|e| format!("BASELINE_NOT_FOUND: {}", e))?;
...
pub fn graph(path: Option<String>) -> Result<WorkspaceGraphResult> {
    ...
    let nodes = raw_nodes
        .into_iter()
        .map(|(id, kind, status)| GraphNode { id, kind, status })
        .collect();
    let edges = raw_edges
        .into_iter()
        .map(|(from, to, relationship)| GraphEdge {
            from,
            to,
            relationship,
        })
        .collect();
    Ok(WorkspaceGraphResult {
        nodes,
        edges,
        path: resolved,
    })
...
pub fn diff_baseline() -> Result<WorkspaceDiffBaselineResult> {
    ...
    let regressions: Vec<WorkspaceRegression> =
        raw_regressions.into_iter().map(to_regression).collect();
    let improvements: Vec<WorkspaceRegression> =
        raw_improvements.into_iter().map(to_regression).collect();
    ...
    Ok(WorkspaceDiffBaselineResult {
        regressions,
        improvements,
        unchanged,
        status,
...
mod tests {
    ...
    fn analyze_instance_with_no_errors_is_healthy() {
        ...
        assert!(
            analysis.is_healthy,
            "instance with no error diagnostics must be healthy"
        );
    }

    #[test]
    fn analyze_missing_state_file_returns_healthy_zero_count() {
        // Non-existent directory: load_from_file bootstraps when the parent dir
        // is writable, so /tmp/<file> would yield a default (non-empty) mesh.
        let svc = WorkspaceService {
            state_path: "/tmp/no-such-dir-lsp-max/ws-analyze/state.json".to_string(),
        };
        let ws = svc.init(".".to_string());
        let analysis = svc.analyze(&ws);
    ...
    fn files_missing_state_returns_empty() {
        let svc = WorkspaceService {
            state_path: "/tmp/no-such-dir-lsp-max/ws-files/state.json".to_string(),
        };
        let (ids, count) = svc.files();
        assert_eq!(count, 0);
        assert!(ids.is_empty());
    ...
    fn graph_missing_state_returns_empty() {
        let svc = WorkspaceService {
            state_path: "/tmp/no-such-dir-lsp-max/ws-graph/state.json".to_string(),
        };
    ...
    fn diff_baseline_fails_when_no_baseline_file_exists() {
        ...
        assert!(
            msg.contains("BASELINE_NOT_FOUND"),
