...
fn main() -> anyhow::Result<()> {
...

    {
        ...
        let mut builder = lsp_max_lsif::lsif_builder::LsifBuilder::new(&mut writer);
        let project_id =
        ...
        for path in collect_files(&args.root, &args.lang) {
            ...
            match args.lang {
                Lang::Rust => lsp_max_lsif::indexer_rust::index_file(&source, &uri, &mut builder)?,
                Lang::TypeScript => lsp_max_lsif::indexer_typescript::index_file(
                    &source,
                    &uri,
    ...
    let final_bytes: Vec<u8> = if args.no_link {
        ...
        lsp_max_lsif::linker::link(&buf, &mut linked)?;
        linked
    };
